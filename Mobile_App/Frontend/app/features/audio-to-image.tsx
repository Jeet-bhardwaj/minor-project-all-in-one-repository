...
import { conversionApi, gridfsApi } from '@/services/api';
...
export default function AudioToImageScreen() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      // Generate userId (masterKey is now generated by backend)
      const generatedUserId = 'user_' + Date.now();
      
      // Save for display
      setUserId(generatedUserId);
      
      // Use new GridFS API v2
      const result = await gridfsApi.audioToImage(
        selectedFile.uri,
        generatedUserId,
        options.compress
      );
      
      console.log('✅ GridFS Conversion result:', result);
      console.log('📦 Conversion ID:', result.conversionId);
      console.log('🖼️  Number of images:', result.numImages);
      
      // Store conversionId in AsyncStorage for Image to Audio screen
      await AsyncStorage.setItem('lastConversionId', result.conversionId);
      await AsyncStorage.setItem('lastUsedUserId', generatedUserId);
      
      // Note: MasterKey is now stored encrypted in MongoDB, not shown to user
      setMasterKey('Stored securely in database');
      ...
      // Create a result object compatible with existing UI
      const compatibleResult = {
        conversionId: result.conversionId,
        numImages: result.numImages,
        totalSize: result.totalSize,
        message: result.message,
        images: [], // Images are now in GridFS, not returned directly
      };
      
      setConversionResult(compatibleResult);
      
      // Show success message with conversionId
      Alert.alert(
        'Success!',
        `Audio converted to ${result.numImages} images\nConversion ID: ${result.conversionId.substring(0, 20)}...`
      );
  ...
  const handleDownloadZip = async () => {
    if (!conversionResult?.conversionId || !userId) {
      Alert.alert('Error', 'Conversion ID or User ID not found');
      return;
    }
    ...
    try {
      ...
      // Fetch ZIP from GridFS via backend
      const zipUrl = `${baseUrl}/v2/conversions/${userId}/${conversionResult.conversionId}/download-zip`;
      
      console.log('📥 Downloading ZIP from GridFS:', zipUrl);

      if (Platform.OS === 'web') {
        // Web: Direct download link
        const link = document.createElement('a');
        link.href = zipUrl;
        link.download = `encrypted_${conversionResult.conversionId}.zip`;
        ...
        await new Promise((resolve, reject) => {
          reader.onloadend = async () => {
            try {
              ...
              const zipFileName = `encrypted_${conversionResult.conversionId}.zip`;
              ...
              // Save the file path for Image to Audio screen
              await AsyncStorage.setItem('lastZipPath', zipUri);
        ...
        Alert.alert(
          '✨ Success!', 
          `ZIP file saved successfully!\n\nLocation: ${FileSystem.documentDirectory}downloads/\n\n${conversionResult.numImages} encrypted images ready for decoding.`,
          [
            {
              text: 'Share File',
              onPress: async () => {
                const zipUri = `${FileSystem.documentDirectory}downloads/encrypted_${conversionResult.conversionId}.zip`;
          ...
          <View style={[styles.mainCard, { backgroundColor: colors.card }]}>
            ...
            {conversionResult && !loading && (
              <View style={styles.resultsContainer}>
                ...
                {/* Download ZIP Button */}
                <TouchableOpacity
                  style={[styles.downloadZipButton, { backgroundColor: colors.tint }]}
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Text style={styles.downloadZipIcon}>📦</Text>
                      <Text style={styles.downloadZipText}>Download ZIP File</Text>
                      <Text style={styles.downloadZipSubtext}>
                        Save all {conversionResult.numImages} encrypted images
                      </Text>
                    </>
                  )}
                </TouchableOpacity>
